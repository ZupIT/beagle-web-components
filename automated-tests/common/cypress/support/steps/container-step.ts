...
Then(/the views that contains the text (.*) (.*) set via context must be displayed/, (ContextValue1, ContextValue2) => {
    containerPage.checkContext(ContextValue1)
    containerPage.checkContext(ContextValue2)
})

Then(/the views that contains the text (.*) (.*) set via context must be displayed/, (FirstAction, SecondAction) => {
    containerPage.checkItems(FirstAction)
    containerPage.checkItems(SecondAction)
})
